...
const CONTACT_COLUMNS = [
  ...
  { key: 'title', header: 'Title' }, { key: 'source', header: 'Source' }, { key: 'status', header: 'Status' },
];
const ACCOUNT_COLUMNS = [
  { key: 'name', header: 'Name' }, { key: 'industry', header: 'Industry' },
  { key: 'website', header: 'Website' }, { key: 'phone', header: 'Phone' }, { key: 'address', header: 'Address' },
];
const DEAL_COLUMNS = [
  { key: 'name', header: 'Name' }, { key: 'stage', header: 'Stage' },
  { key: 'value', header: 'Value' }, { key: 'probability', header: 'Probability' }, { key: 'expectedCloseDate', header: 'Expected Close Date' },
];

export function ImportExportView() {
  const { contacts, bulkImportContacts } = useContacts();
  const { accounts, bulkImportAccounts } = useAccounts();
  const { deals, bulkImportDeals } = useDeals();
  ...
  const handleImport = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (ev) => {
      ...
      if (rows.length === 0) { setImportResult({ count: 0, errors: ['No data rows found in CSV'] }); return; }
      const errors: string[] = [];
      try {
        if (importType === 'contacts') {
          const mapped = rows.map((r, i) => {
            if (!r['First Name']) errors.push(`Row ${i + 2}: Missing First Name`);
            return { firstName: r['First Name'] || '', lastName: r['Last Name'] || '', email: r['Email'] || '', phone: r['Phone'] || '', title: r['Title'] || '', source: r['Source'] || '', status: (r['Status'] || 'prospect') as any, accountId: undefined, accountName: undefined };
          });
          if (errors.length === 0) bulkImportContacts(mapped);
        } else if (importType === 'accounts') {
          const mapped = rows.map((r, i) => {
            if (!r['Name']) errors.push(`Row ${i + 2}: Missing Name`);
            return { name: r['Name'] || '', industry: r['Industry'] || '', website: r['Website'] || '', phone: r['Phone'] || '', address: r['Address'] || '' };
          });
          if (errors.length === 0) bulkImportAccounts(mapped);
        } else if (importType === 'deals') {
          const mapped = rows.map((r, i) => {
            if (!r['Name']) errors.push(`Row ${i + 2}: Missing Name`);
            return { name: r['Name'] || '', stage: (r['Stage'] || 'prospecting') as any, value: Number(r['Value']) || 0, probability: Number(r['Probability']) || 0, expectedCloseDate: r['Expected Close Date'] || '', accountId: undefined, accountName: undefined, contactId: undefined, contactName: undefined };
          });
          if (errors.length === 0) bulkImportDeals(mapped);
        }
        setImportResult({ count: errors.length === 0 ? rows.length : 0, errors });
      } catch (err: any) { setImportResult({ count: 0, errors: [err.message] }); }
    };
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-bold text-foreground flex items-center gap-2"><FileSpreadsheet className="h-6 w-6 text-primary" />Import / Export</h1>
        <p className="text-muted-foreground mt-1">Import and export CRM data as CSV files</p>
      </div>
      <div className="grid md:grid-cols-2 gap-6">
        <Card>
          <CardContent className="py-6 space-y-4">
            <h2 className="text-lg font-semibold text-foreground flex items-center gap-2"><Download className="h-5 w-5" /> Export Data</h2>
            <div>
            ...
            <Button onClick={handleExport} className="w-full"><Download className="h-4 w-4 mr-2" />Export as CSV</Button>
          </CardContent>
        </Card>
        <Card>
          <CardContent className="py-6 space-y-4">
            <h2 className="text-lg font-semibold text-foreground flex items-center gap-2"><Upload className="h-5 w-5" /> Import Data</h2>
            <div>
              <Label>Entity Type</Label>
              <Select value={importType} onValueChange={v => { setImportType(v as ExportEntity); setImportResult(null); }}>
                <SelectTrigger><SelectValue /></SelectTrigger>
                <SelectContent><SelectItem value="contacts">Contacts</SelectItem><SelectItem value="accounts">Accounts</SelectItem><SelectItem value="deals">Deals</SelectItem></SelectContent>
              </Select>
            </div>
            <div>
                ...
                {importResult.errors.length > 0 ? (
                  <div className="flex items-start gap-2">
                    ...
                    <div>
                      ...
                      {importResult.errors.slice(0, 5).map((err, i) => <p key={i} className="text-sm text-destructive">{err}</p>)}
                      {importResult.errors.length > 5 && <p className="text-sm text-destructive">...and {importResult.errors.length - 5} more errors</p>}
                    </div>
                  </div>
                ) : (
                  <div className="flex items-center gap-2"><CheckCircle2 className="h-5 w-5 text-green-600" /><span className="font-medium text-green-700 dark:text-green-400">Imported {importResult.count} records</span></div>
                )}
              </div>
            )}
          </CardContent>
